...
public class Setup {
    ...
    private static void checkNpm() throws IOException, Exception {
        ProcessBuilder builder = new ProcessBuilder("sh", "-c","npm -v");
    ...
    private static boolean checkGeth() throws Exception {
        ProcessBuilder builder = new ProcessBuilder("sh", "-c","geth  version");
...
//        String command = "solcjs --bin --include-path @openzeppelin  --base-path "+contractName+".sol";
        String command = "solcjs --bin "+contractName+".sol";
        ProcessBuilder builder = new ProcessBuilder("sh", "-c",command);
        builder.redirectErrorStream(true);
        Process p = builder.start();
        BufferedReader r = new BufferedReader(new InputStreamReader(p.getInputStream()));
